...
/* 즉 inner join은 기준 컬럼의 값이 일치할때만 데이터를 연결한다.*/
 ...
 * 강의실이 아직 지정되지 않은 강의의 경우 강의실 테이블의 데이터와 연결하지 못 해
 * 결과로 출력되지 않는 문제가 있다.*/
...
select 
    ...
    r.loc,
    r.name
from lect left outer join room r on l.rno=r.rno;
