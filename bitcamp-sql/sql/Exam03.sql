...
/* 가상의 컬럼 값을 조회하기*/
select no, concat(name,'(',tel,')') from test1;
```
...
select (4=5), (4!=5), (4>5), (4>=5), (4<5), (4<=5), (4<>5) from dual;
/* 다른 DB에서는 from dual 써야 가능! mysql에서는 안써도 됨 */
```
...
insert into test18 (title, regdt) values('aaaa', '2017-11-22');

/* 다음 형식의 문자열을 날짜 값으로 지정할 수 없다.*/
insert into test18 (title, regdt) values('bbbb', '11/22/2017');

/* 위 형식의 문자열을 날짜 값으로 저장하려면 str_to_date() 함수를 사용해야 한다.*/
insert into test18 (title, regdt) 
