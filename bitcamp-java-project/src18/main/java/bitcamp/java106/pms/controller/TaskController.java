...
public class TaskController {
    ...
    private void onTaskAdd(final Team team) { 
        // 개발자들의 실수를 막기 위해 파라미터 변수를 final 로 고정시켜놓는다
        // ex) team = new Team(); 이런식으로 새로운 인스턴스 못넣음!
        ...
        if (str.length() == 0) {
            ...
            if (date.getTime() < team.getStartDate().getTime()) {
                // 사용자입력(data) 시간이 기본값(team) 시간보다 작다면 기본값으로 출력
                task.setStartDate(team.getStartDate());
            } else {
            task.setStartDate(date); // 아니라면 사용자 입력 시간을 출력한다.
            }
        }
        ...
        if (str.length() == 0) {
            ...
            if (date.getTime() > team.getEndDate().getTime()) {
                // 사용자입력(data) 시간이 기본값(team) 시간보다 크다면 기본값으로 출력
                task.setEndDate(team.getEndDate());
            } else {
            task.setEndDate(date); // 아니라면 사용자 입력 시간을 출력한다.
            }
        }
        ...
        if (memberId.length() != 0) {
            if (!teamMemberDao.isExist(team.getName(), memberId)) { // 존재하지 않는다면
                System.out.printf("'%s'는 이 팀의 회원이 아닙니다. 작업자는 비워두겠습니다." , memberId);
            } else {
                
            task.setWorker(this.memberDao.get(memberId)); 
            // memberid 줄테니까 멤버 정보 주면 task에 저장
            }
        }
        
        taskDao.insert(task);
        // System.out.println(task);  
        // 기본정보 출력-> 해시코드 안나오게하기 -> member,task,team.java 에 오버라이딩
    ...
    private void onTaskList(final Team team) {
        ...
        for (Task task : tasks) {
            // Task 의 task 를 준비하고, 배열 tasks 를 순서대로 현재 task 에 넣어라
            System.out.printf("%d, %s, %s, %s, %s \n",
                    task.getNo(), task.getTitle(), task.getStartDate(),
                    task.getEndDate(), (task.getWorker() == null) ?
