...
// => 그러나 이 인터페이스르르 직접 구현하려면 많은 메서드(12개)를 모두 구현해야 한다.
//    너무 번거롭다.
// => 그래서 자바는 프로퍼티 에디터를 미리 구현한 PropertyEditorSupport 라는 클래스를 제공한다.
//    따라서 인터페이스를 직접 구현하지 말고 이 클래스를 상속받아라!
//
// 1) java.beans.PropertyEditorSupport 를 상속 받는다.
