...
// 실행 순서와 메모리
// 1) java -cp bin step06.Exam04_1
//    => JVM은 클래스 정보를 Method Area 영역에 로드한다.
// 2) main() 호출
//    => JVM Stack 영역에 main() 메서드가 사용할 로컬 변수 준비한다.
// 3) swap() 호출
//    => JVM Stack 영역에 swap() 메서드가 사용할 로컬 변수 준비한다.
// 4) swap() 실행 완료
//    => JVM Stack 영역에 있던 swap()이 사용한 메모리를 제거한다.
// 5) main() 실행 완료
//    => JVM Stack 영역에 있던 main()이 사용한 메모리를 제거한다.
// 6) JVM 실행 종료
//    => JVM 이 사용한 모든 메모리를 OS 에 반납한다.
...
// 3) JVM Stack
//    => 스래드 별로 Stack 메모리를 따로 관리한다.
//    => 메서드의 로컬 변수는 둔다.
//    => 각 메서드마다 프레임 단위로 관리한다.
//    => 메서드 호출이 끝나면 그 메서드가 사용한
//    => 프레임 메모리가 제거된다.
//
// JVM 이 종료하면 JVM이 사용했던 모든 메모리는 OS에 반납한다.