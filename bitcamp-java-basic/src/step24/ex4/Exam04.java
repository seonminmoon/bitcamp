// 스레드 생명주기(lifycycle) - running 상태 CPU 쟁탈전
package step24.ex4;
...
public class Exam04 {
    public static void main(String[] args) throws Exception {
        ...
        //    물론 OS가 배분한 후 임의시간 후에 다시 회수하여 
        //    다른 스레드(현재 스레드 포함)나 프로세스에 배분한다.
...
// 프로세스(스레드) 스케줄링
// => OS가 정책에 따라 프로세스나 스레드에 CPU 사용을 배분하는 정책
// 1) Round-Robin 방식
...
//    - 우선 순위 배분 방식에서는 우선순위가 낮은 경우 실행에서 소외되는 문제가 발생하기 때문에
//      우선 순위가 높은 프로세스나 스레드 때문에 실행 순서가 밀릴 때 마다
//      원래의 낮은 우선순위를 높임으로써(aging) 결국에는 모든 프로세스와 스레드가
//      실행을 완료할 수 있게 한다.
//
// "컨텍스트 스위칭(context switching)"
// - 동시에 여러 개의 프로세스나 스레드를 실핼할 때
//   CPU 사용권을 뺏어 다른 프로세스나 스레드에게 주기 전에
//   현재까지 실행한 코드의 위치정보를 저장해야 한다.
