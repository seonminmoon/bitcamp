...
public class StatelessServer2 {
    ...
    static HashMap<Integer, Integer> sesstionMap = new HashMap<>();
    ...
    static void processRequest(Socket socket) throws Exception {
        try (
            ...
            // => "\n123\n" : 123번 클라이언트의 결과를 달라는 의미다.
            
            if (str.equals("")) {
                out.println("결과 = " + sesstionMap.get(clientId));
                return;
            }
            
            // 클라이언트 아이디가 0일 때,
            // 아직 서버로부터 클라이언트 식별 번호를 발급 받지 않은 상태이기 때문에
            // 새 번호를 발급한다.
            // 예를 들어 클라이언트가 다음과 같은 데이터를 보냈다면,
            // => "100\n0\n"
            if (clientId == 0) {
                clientId = ++countClient;
                sesstionMap.put(clientId,0); 
            ...
            int sum = sesstionMap.get(clientId);

            // 기존 값에 새 값을 더하여 저장한다.
            sesstionMap.put(clientId, sum + value);
