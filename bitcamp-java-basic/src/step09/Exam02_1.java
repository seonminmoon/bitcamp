...
public class Exam02_1 {
    public static void main(String[] args) throws Exception {
        ...
        System.out.println(s1.compareTo("Hello")); // 같을땐 0, 아닐땐 음수나 양수
        ...
        System.out.println(s1); // 기존 인스턴스 값은 변경하지 않는다.
        System.out.println(s2); // 새로 문자열을 만든다.
        
        // 두 인스턴스에 들어 있는 문자열이 같은지 비교할 때
        ...
        //    이클립스에서 JVM을 실행하면 JVM의 기본 문자집합은 UTF-8이기 때문에
        //    바이트 배열에 코드 값으로 지정한다.
        ...
        bytes = s3.getBytes("EIF");
        for (int i = 0; i < bytes.length; i++)
            System.out.printf("%x,", bytes[i]);
            System.out.println();
            
            //String 클래스에도 특정 인스턴스가 아닌 일반 목적으로
            // 문자열을다룰ㄹ 수 있는 메서드를 제공한다.
