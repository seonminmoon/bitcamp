...
public class Exam04_4 {
    ...
    // HTTP 요청에서 "Content-Type" 헤더의 값에 따라 호출될 메서드 구분하기
    // => 클라이언트는 POST 요청으로 데이터를 보낼 때
    //    서버에 그 데이터 타입을 알려줘야 한다.
    //    그 떄 사용하는 헤더가 "Context-Type" 이다.
    // 테스트 방법: 
    // 예1) exam04_4.html
...
name=aaa&age=20&tel=1111-2222&postno=19742&addr=서울시강남구
*/
    // 만약 클라이언트가 보낸 데이터가 "변수=값&변수=값&변수=값" 형태로 되어 있다면?
    ...
    // 만약 클라이언트가 보낸 데이터가 "값,값,값,값" 형태로 되어 있다면?
    ...
    // 만약 클라이언트가 보낸 데이터가 
    // {"name":"aaa","aaa":20,"tel":"1111-2222"} 형태로 되어 있다면?
